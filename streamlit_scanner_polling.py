"""
Streamlit Live Scanner Dashboard - REST POLLING version
=============================================================
Same S/R + RVOL + VWAP pattern + OI + aggression scoring logic as
streamlit_scanner_banknifty.py, but fetches live data via REST polling
(Upstox's Full Market Quotes endpoint, all ~208 stocks in one call every
10s) instead of WebSocket streaming - built after extensive testing found
the WebSocket connection opened successfully but never delivered tick
data, while REST calls worked reliably throughout the same session.

RUN THIS WITH (not plain python!):
    streamlit run streamlit_scanner_polling.py
...
The background polling/scoring must NOT restart on every rerun. The fix
is st.cache_resource, Streamlit's built-in mechanism for objects that
must be created exactly once and persist across reruns and browser
sessions - the UI simply reads the shared state on each rerun and
redraws the tables; it never touches the network itself.

Auto-refresh is done with a simple sleep + st.rerun() loop at the bottom
of the script (no extra package needed).

Setup requirements (simpler than the WebSocket version - no protobuf!):
...
def fetch_all_quotes(instrument_keys, access_token):
    ...
    instruments at once (up to 500 per Upstox's limit). Returns raw
    response dict, matched to states via each entry's 'instrument_token'
    field (not the outer 'EXCHANGE:SYMBOL' key)."""
...
def update_states_from_quotes(quotes_data, states_by_key):
    ...
    for entry in quotes_data.values():
        ...
        if state is None:
            continue
        state.update(
            entry.get("last_price"), entry.get("average_price"),
            entry.get("volume") or 0,
            entry.get("total_buy_quantity"), entry.get("total_sell_quantity"),
        )
        state.update_oi(entry.get("oi"))
        updated_count += 1
    ...
    try:
        ...
        with _app_lock:
            if _app["generation"] != my_generation:
                return
            _app["states"] = states
            _app["status"] = f"prep loaded ({len(states)} symbols), starting REST polling"
        ...
        with _app_lock:
            if _app["generation"] != my_generation:
                return
            _app["status"] = "connected"  # "connected" here means polling is active

        last_poll_time = 0
        # Loop runs every POLL_INTERVAL_SEC (fetching fresh REST quotes each
        # time), but only re-scores/re-evaluates every SCORING_INTERVAL_SEC -
        # these are independent cadences, gated separately below. Replaces
        # the WebSocket path, which was confirmed via extensive testing to
        # open successfully but never actually deliver tick data, while
        # REST calls worked reliably throughout the same session.
        last_score_time = 0
        while True:
            with _app_lock:
                if _app["generation"] != my_generation:
            ...
            now_ts = time.time()
            if now_ts - last_poll_time >= POLL_INTERVAL_SEC:
                try:
                    quotes = fetch_all_quotes(instrument_keys, access_token)
                    update_states_from_quotes(quotes, states_by_key)
                    last_poll_time = now_ts
                except Exception as e:
                    with _app_lock:
                        if _app["generation"] == my_generation:
                            _app["error"] = f"Polling error: {type(e).__name__}: {e}"

            if now_ts - last_score_time >= SCORING_INTERVAL_SEC:
...
# STREAMLIT UI
# ---------------------------------------------------------------------------
st.set_page_config(page_title="F&O Live Scanner (REST Polling)", layout="wide")
st.title("F&O Live Scanner (REST Polling)")
...
with st.expander("Debug info (click if something looks stuck)"):
    ...
    st.write("Mode:", "REST polling (no protobuf needed)")
